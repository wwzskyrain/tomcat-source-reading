...
//      如果process线程还没有从await阻塞中返回，没有接受上一次赋值socket，没有将available设置为false，
//      那么这里"connector"线程也就会阻塞了。
        while (available) {
            try {
                wait();
    ...
    private synchronized Socket await() {
        ...
        // 等着，只到Connector提供了一个Socket.
        while (!available) {
            try {
                wait();
//  谁都可以唤起这个线程，但是它就等available这个信号。
//  如果不是available这个信号被设置之后的唤醒，它还继续等候——痴心呐。
//  这么痴心的线程，Connector唤醒线程的时候一定不要忘了设置available信号啊，不然唤醒也是徒劳啊。
        ...
        available = false;
        notifyAll();    //处理器拿到连接器给的socket后，立刻（在处理之前）就notify连接器线程了。
    ...
    private void parseConnection(Socket socket)
        ...
        //socket是谁来创建的？类比tomcat是servlet容器的话，JVM就是一个class容器。
     ...
     * swallowed and dealt with. 哦，why ？
     *
...
//                  原来500就是在这里设置的哈。
    ...
    public void run() {
        ...
        while (!stopped) {
            ...
            //http请求处理完毕后，将Processor返回给"处理器池"，
            // 这时，Processor这个线程还在while中的。而且，应该就'等候'在"等待connector赋值socket"。
